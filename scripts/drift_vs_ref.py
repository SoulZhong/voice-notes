...
def norm_xcorr_at(a, b, t0, win, lo, hi):
    ...
    if rest.size and float(rest.max()) > 0.9 * best:
        return None
    # 显著性:峰要高出背景涨落。MAD≈0 时**不能**一律拒(Codex 十轮 P2):稀疏内容
    # (单个脉冲)的正确滞后可以是 1.0 而其余滞后是同一个小常数,那是唯一峰而非并列峰
    # ——这种情形交给上面的唯一性检查裁决即可。只有 MAD 有意义时才做 z 检验。
    if mad >= 1e-9 and (best - med) / mad < PEAK_Z:
        return None
...
def main():
    ...
    print(
        "提醒:与**外部参考文件**比时,这条斜率 ≈ 补偿残余 + (R_master − R_播放侧)/标称率"
        "——master 的误差是加、播放侧的误差是减,两者都得独立量到才谈得上分离残余,"
        "而播放侧时钟通常没人量过。注意 master 误差**在这里不是共模**(共模只出现在"
        "mic vs system 的声学口径里),别照搬那套抵消说法。判据 1 要用声学口径量。"
    )
