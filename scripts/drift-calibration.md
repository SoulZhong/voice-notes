...
1. 准备刺激音:
   ...
   ```sh
   python3 scripts/gen_calibration_clicks.py clicks.wav --minutes 12
   ```

   用系统播放器循环播放这个文件即可。

   **不要用等间隔的节拍器/click track**:周期性刺激的互相关在 ±半周期处会出现
   同样高的旁瓣,延迟估计可能整体锁到错的那一支——E1 标定实测撞上过,500ms 周期
   下 440s 那一窗给出 +198ms 的离群值,恰是半个周期。生成器用伪随机间隔
   (0.30-0.75s 均匀抖动,种子固定可复现)消除这个歧义:除零点外没有第二个高峰,
   窗内延迟只有一个解。
...
4. 互相关真值(调用顺序为 system.wav 在前、mic.wav 在后 —— 这样输出的符号才直接对齐
   ...
   cargo run --features devtools --bin xcorr_align -- <note_dir>/system.wav <note_dir>/mic.wav
