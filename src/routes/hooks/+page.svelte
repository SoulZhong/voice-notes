...
<div class="page">
  <header class="topbar"><h1>{t("hooks.title")}</h1></header>

  <p class="intro">{t("hooks.intro")}</p>

  <section>
    <h2 class="section-title">{t("hooks.events.title")}</h2>
    <div class="flow-card">
      <svg class="flow-svg diagram" viewBox="0 0 820 286" role="img" aria-label={t("hooks.flow.aria")}>
        <defs>
        ...
        <g class="node">
          ...
          <text class="node-label" x="140" y="92" text-anchor="middle" dominant-baseline="central">{t("hooks.flow.recording")}</text>
        </g>
        <g class="node">
          <rect class="node-box" x="285" y="63" width="120" height="58" rx="10" />
          <text class="node-label" x="345" y="92" text-anchor="middle" dominant-baseline="central">{t("hooks.flow.recorded")}</text>
        </g>
        <g class="node">
        ...
        <g class="node">
          ...
          <text class="node-label" x="140" y="234" text-anchor="middle" dominant-baseline="central">{t("hooks.flow.paused")}</text>
        </g>

        <!-- 边标签(中文名 + 事件键) -->
        <text class="edge-label" text-anchor="middle" x="52" y="42">
          <tspan class="cn">{t("hooks.event.recordingStarted")}</tspan>
          <tspan class="key" x="52" dy="15">recording_started</tspan>
        </text>
        <text class="edge-label" text-anchor="middle" x="242" y="42">
          <tspan class="cn">{t("hooks.event.recordingStopped")}</tspan>
        ...
        <text class="outcome-label" text-anchor="middle" x="636" y="61">{t("hooks.flow.success")}</text>
        <text class="outcome-label" text-anchor="middle" x="636" y="141">{t("hooks.flow.failure")}</text>
        <text class="edge-label" text-anchor="end" x="116" y="151">
          <tspan class="cn">{t("hooks.event.recordingPaused")}</tspan>
          <tspan class="key" x="116" dy="15">recording_paused</tspan>
        </text>
        <text class="edge-label" text-anchor="start" x="165" y="184">
          <tspan class="cn">{t("hooks.event.recordingResumed")}</tspan>
          <tspan class="key" x="165" dy="15">recording_resumed</tspan>
        </text>
      </svg>
      <p class="flow-caption">{t("hooks.flow.caption")}</p>
    </div>
  </section>

  <section>
    <h2 class="section-title">{t("hooks.env.title")}</h2>
    <div class="rows">
      <div class="row">
        <div class="row-info">
          <span class="row-label"><code>VN_EVENT</code></span>
          <span class="row-desc">{t("hooks.env.event")}</span>
        </div>
      </div>
      <div class="row">
        <div class="row-info">
          <span class="row-label"><code>VN_NOTE_ID</code></span>
          <span class="row-desc">{t("hooks.env.noteId")}</span>
        </div>
      </div>
      <div class="row">
        <div class="row-info">
          <span class="row-label"><code>VN_NOTE_TITLE</code></span>
          <span class="row-desc">{t("hooks.env.noteTitle")}</span>
        </div>
      </div>
      <div class="row">
        <div class="row-info">
          <span class="row-label"><code>VN_NOTE_TEXT</code></span>
          <span class="row-desc">{t("hooks.env.noteText")}</span>
        </div>
      </div>
      <div class="row">
        <div class="row-info">
          <span class="row-label"><code>VN_NOTE_STARTED_AT</code> / <code>VN_NOTE_ENDED_AT</code></span>
          <span class="row-desc">{t("hooks.env.noteTimes")}</span>
        </div>
      </div>
      <div class="row">
        <div class="row-info">
          <span class="row-label"><code>VN_NOTE_DURATION_SECS</code></span>
          <span class="row-desc">{t("hooks.env.duration")}</span>
        </div>
      </div>
      <div class="row">
        <div class="row-info">
          <span class="row-label"><code>VN_NOTE_SPEAKERS</code></span>
          <span class="row-desc">{t("hooks.env.speakers")}</span>
        </div>
      </div>
      <div class="row">
        <div class="row-info">
          <span class="row-label"><code>VN_NOTE_TEXT_TRUNCATED</code></span>
          <span class="row-desc">{t("hooks.env.truncated")}</span>
        </div>
      </div>
    </div>
  </section>
  ...
  <section>
    <h2 class="section-title">{t("hooks.webhook.title")}</h2>
    <pre class="snippet">{t("hooks.webhook.sample")}</pre>
    <p class="hint">{t("hooks.webhook.hint")}</p>
  </section>
</div>
