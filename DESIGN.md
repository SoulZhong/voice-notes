...
- **图谱页**(侧栏第三页签「图谱」,主从结构;消费 Aing 知识图谱):侧栏=master 实体列表——顶部 `.search` 搜索框(按名/别名子串过滤)+ `gchips`(kind 过滤药丸,窄栏紧凑换行:`surface-press` 底、选中 `accent-tint`+`accent` 字;「全部」+ 数据里实际出现的 kind 动态生成,`kindLabel` 中文化,未知 kind 原样);列表复用侧栏 `.item`/`.dot`/`.main-line`(人实体色点=`speakerColor` 会议搭子色系、非人=`hairline-strong` 中性点;meta 一行「kind · N 笔 · M 提及」),按出现笔记数降序(SQL 稳定序,不依赖 read_dir),点非人经 `/graph?e=<gid>` 深链、点人跳 `/speakers/[person_id]`。主区=stage:未选实体时渲染 **force-graph**(见下)或(节点<2/无边时)居中引导占位,无实体时空态引导(「配置大模型并重新 Aing 以启用知识图谱」+ 去 AI 设置)。**entity-panel**(主区详情,选中实体):顶部 `.back` 文字链接「← 返回图谱」(清 `?e=` 深链参数退回力导图/占位视图)+ 单列居中 `max-width 640`——大标题(20px 500)+ kind 徽章 + 别名行(`ink-secondary`,超 6 个折叠「等 N 个」,抵御 ASR 同音异写被误并的噪声)+ 「出现在 N 篇·M 提及」caption;两节竖排「出现的笔记」(行=标题+「日期·N 提及」meta,点开 `/notes/[id]`)与「相关实体」(共现,行=色点+名+kind 徽章+右对齐「N 篇共现」,点非人切换面板、点人跳会议搭子)。**注**:面板列出的笔记数可能 < note_count(标题查不到的笔记跳过),属正常优雅降级;实体点击命中 `null`(声纹 ingest 后变更、尚未入图)当「尚未入图」提示而非报错(同样带返回入口)。
