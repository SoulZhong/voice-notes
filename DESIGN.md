...
- 实体/文章双视角、搜索、关系类型/kind 筛选、逐层展开、收起与 evidence 回看共同服务探索；普通点击查看实体详情，右键进入 rename、alias、merge、split、suppress、restore、end/create relation 与 undo 等可逆治理。两点路径与待整理队列不进入日常界面。
...
- **分析说话人**(会议搭子整理,三面入口一个队列):高置信归属建议(裸余弦 ≥0.74 或 S-Norm z≥3,且 loser 未命名、winner 已命名、不在落盘拒绝名单)在录制停止/启动/库变化时**自动归并**——每条合并前把双方快照+样本副本落 `merge_journal/`(上限 50 条),单条可撤销;后续触及双方的操作使条目失效(由另一次合并失效的,撤销那次合并即复活=LIFO 链式撤销);撤销过的 pair 落盘拒绝名单,自动归并不再碰。拿不准的在**会议搭子概览页(`/speakers`)下方常驻分析区**逐条审阅:全量列表,队列=自动归并回执→归属建议→同名组→无样本条目,卡上给双方全部样本试听(带日期)+最近 3 场会议+相似度;动作 合并/忽略(保留),回执卡 好/撤销(失效变灰注明原因);试听单实例互斥切卡即停;录制中只读浏览。**入口收敛**:①侧栏「概览与整理」行徽标=队列总数(warning 药丸,0 隐藏);②概览页下方常驻分析区(N 件待处理 · M 条已自动归并,原位展开无需跳转,结果驻留不因进页重算);③详情页头部 ctx-card 照旧(建议跟人走,行内合并/忽略),手动合并后给「已合并·撤销」条。四处同源(`src/lib/tidy.svelte.ts` 会话态+`tidyQueue.ts` 纯队列;忽略/保留落盘 dismissed.json,拒绝名单落盘)。
...
- **图谱页**（侧栏第三页签「图谱」，消费 Aing 语义知识图谱）：它是笔记关系的轻量地图，不是图数据库工作台。保留实体/文章双视角、实体搜索与类型、关系类型筛选、渐进披露的时间/历史/弱连接筛选、点击节点逐层展开、实体详情、右键治理与关系证据；不提供两点路径、待整理队列或专家级批量工作流。语义索引不可用或待补建时明确显示降级原因，并提供「补建语义关系」入口；失败不会阻断笔记录制、保存、精修或阅读。
- **force-graph**（图谱页主区，d3-force + SVG 自绘，`ForceGraph.svelte`）：顶点和边直接承载**完整可读标签**，不使用省略号或脱离顶点的常驻说明方框；长名称在顶点内多行排布，常用规模下关系名沿边直接显示，大图缩放时逐级显示更多完整的顶点名称和关系标签。明确关系使用高对比 accent 粗实线与箭头，弱连接使用低饱和次级色细实线，避免虚线在密集网络中制造碎片噪声；文章视角直接写“2 个共用实体”，实体视角写“2 篇笔记同时提到”。常驻图例按当前视角解释边的语义。别名继续用于搜索，证据与治理状态通过详情面板查看。顶点可拖拽，空白处平移，滚轮围绕光标缩放；hover 聚焦邻域，点击节点展开一层，键盘可达，`prefers-reduced-motion` 时直接定格。大图默认只显示关系主干，不以截断文本换取空间。
