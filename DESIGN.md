...
- **智能精修配置**(AI 页首卡,.config 内嵌面板):首行「执行方式」segmented 二选一——在线接口 / 本机 Agent。在线接口态:一键填充 `button-secondary` 预设行 + 三字段(接口地址/模型/API Key);本机 Agent 态:「Agent」segmented 四选一(Claude Code/Codex/Gemini/Cursor,标签尾随探测记号 ✓/⚠,title 给路径或指引),两个可选字段(模型/CLI 路径,占位符给探测结果或示例),config-hint 按探测结果给「已检测到 <路径>+一句话原理」或「未检测到 → 先装并登录或指定路径」。启用总开关仍在设置页「录制」区。
