...
- **图谱页**(侧栏第三页签「图谱」,主从结构;消费 Aing 知识图谱):**两视角切换**——侧栏顶部 `.gmode` 分段控件「实体 / 文章」(`surface-press` 底 + 选中项 `surface` 白实底 + `shadow-btn`,同设置页 segmented 语言;`graphFilter.mode` 跨组件共享态)。**实体视角**(默认):节点=实体、边=共现(两实体共享几篇笔记),侧栏是实体列表 + kind 过滤药丸,主区可选中实体开详情面板(见下 entity-panel)。**文章视角**:节点=笔记、边=共享实体(两篇笔记共提到几个相同实体),是实体视角的**对偶**——同一份 `note_entities` 表,只是把"谁连谁"的两端对调(后端 `note_nodes`/`note_shared_edges`,前端 `noteGraphData` → 复用同一 `GraphData`/`ForceGraph`,节点 `kind:"note"`、`name`=标题、`note_count` 字段复用为该笔记的实体数以驱动节点大小);侧栏改笔记列表(标题 + 「N 个实体 · M 提及」meta,搜索按标题),主区力导图**无详情面板、无 kind 过滤、无图例**(`showLegend={false}`,单一节点类型图例无信息量),点节点直接进 `/notes/[id]`(笔记页本身承载相关笔记/实体高亮);文章视角数据懒加载(首次切到才拉,之后缓存)。侧栏=master 实体列表(实体视角下)——顶部 `.search` 搜索框(按名/别名子串过滤)+ `gchips`(kind 过滤药丸,窄栏紧凑换行:`surface-press` 底、选中 `accent-tint`+`accent` 字;「全部」+ 数据里实际出现的 kind 动态生成,`kindLabel` 中文化,未知 kind 原样);列表复用侧栏 `.item`/`.dot`/`.main-line`(人实体色点=`speakerColor` 会议搭子色系、非人=`hairline-strong` 中性点;meta 一行「kind · N 笔 · M 提及」),按出现笔记数降序(SQL 稳定序,不依赖 read_dir),点非人经 `/graph?e=<gid>` 深链、点人跳 `/speakers/[person_id]`。**侧栏顶部固定「全局图谱」行**(紧跟录制按钮,不随实体列表滚动消失,点击清 `?e=` 深链退回力导图总览,详情态下从任何深度都能一键退回)——**描边幽灵按钮而非常驻实底**:未选中实体是常态,若借「概览与整理」那套 `.item.current` 实底高亮会让这行几乎永远顶一块深色卡片、比白色录制药丸还抢眼(冒烟反馈"不够美观");但纯文字链接(无边框,仅 hover 变色)又太朴素,看不出是可点元素(冒烟反馈)。终稿=`hairline` 描边 + 透明底(`ink-faint` 图标 + `ink-secondary` 字),hover 时描边加深 `hairline-strong` + `surface-soft` 底 + 字色变 `ink`,兼顾"看得出能点"与"不抢录制药丸的视觉重量"。主区=stage:未选实体时渲染 **force-graph**(见下)或(节点<2/无边时)居中引导占位,无实体时空态引导(「配置大模型并重新 Aing 以启用知识图谱」+ 去 AI 设置)。**entity-panel**(主区详情,选中实体):顶部 `.back` **药丸按钮**「← 返回图谱」(冒烟反馈原纯文字链接太不起眼——`surface-soft` 底+`hairline` 描边+14px 字号,与其他主要操作按钮同一量级;清 `?e=` 深链参数退回力导图/占位视图;侧栏「图谱」页签同样能退——点击判断需同时看 pathname 与 search,只看 pathname 会漏判详情态)+ `.detail` flex 列撑满 `height:100%`(居中 `max-width 1400`,不再是内容撑多高算多高的文档流)——头部固定不参与拉伸:大标题(20px 500)+ **改名按钮紧跟名字右侧**(冒烟反馈原 `margin-left:auto` 甩到整行最右端跟名字视觉脱节,现直接排在名字后、kind 徽章前)+ kind 徽章 + 别名行(`ink-secondary`,超 6 个折叠「等 N 个」,抵御 ASR 同音异写被误并的噪声)+ 「出现在 N 篇·M 提及」caption;下方 `.d-cols`(flex 行,填满剩余高度,窄屏 `<760px` 退竖排)分两栏——**左栏「出现的笔记」**(固定宽 340px,标题旁总数徽章,`.d-scroll` flex:1 内部滚动,行=标题+「日期·N 提及」meta,点开 `/notes/[id]`;文字列表,日期/标题本质要读,不图形化);**右栏「相关实体」是可视化,不是文字列表**——标题旁总数徽章 + 超 `EGO_MAX_RELATED`(30)时的「只画最相关的 N 个」说明,主体 `.ego-wrap`(`flex:1` 撑满剩余画布,`hairline` 描边卡片)内渲染**以当前实体为中心的小型力导图**(复用 `ForceGraph.svelte`:中心节点+最相关的 30 个,边=中心↔相关的共享笔记数 ∪ 相关实体彼此间的全局共现边,展示邻居聚簇而非纯星形;`minEdgeWeight=1`/`backboneK=999` 覆盖全局图默认值——中心的直连关系已被后端筛过,不该被全局噪声阈值误伤);节点点击=`pickRelated`(人跳会议搭子、非人 `goto` 切换 `?e=` 到该实体,详情面板随之刷新)。`ForceGraph` 组件参数化为 props(`maxNodes`/`minEdgeWeight`/`backboneK`/`truncHint`,默认值即原全局图用值)+ 内部改 `$effect` 响应式重建仿真(而非一次性 `onMount`),使同一组件实例在详情页切换选中实体时正确换图而非停留旧数据。**注**:面板列出的笔记数可能 < note_count(标题查不到的笔记跳过),属正常优雅降级;实体点击命中 `null`(声纹 ingest 后变更、尚未入图)当「尚未入图」提示而非报错(同样带返回入口)。
- **force-graph**(图谱页主区,d3-force + SVG 自绘,`ForceGraph.svelte`):节点=实体、边=共现(权重=共享笔记数)。**规模封顶**(默认视图):926+ 实体不可全画,只取「共享≥2篇」的强边算度数,按 note_count 降序取前 60 节点,每节点只保留最强 3 条边(backbone 稀疏化,把千级共现边收成可读骨架)、超出角标提示「显示连接最紧的 N 个」+ **「展开一层」/「显示全部」两个按钮**。「显示全部」一步到位解除全部封顶(边权重下限降到 1、backbone 放宽到 6、节点数上限 2000 安全兜底),配合缩放/平移浏览大图。「展开一层」是渐进式的——以当前骨架图为种子逐层并入邻居,可重复点「继续展开」;**每个节点每层最多带入 8 个权重最高的新邻居**而非来者不拒(种子里的枢纽实体如「AI」共现过几百个不同实体,不设上限的话点一次就能把 95% 的图拖进来,等于变相的「显示全部」,完全违背"避免一下爆炸"的初衷),实测 60→178→335 的渐进曲线。三态提示条:骨架图态「显示连接最紧的 N 个 · 展开一层 · 显示全部」→ 展开态「已展开 K 层 · 共 N 个实体 · 继续展开 · 显示全部 · 收起」→ 全部态「已显示全部 N 个 · 收起」,收起统一回到默认骨架图。**大图(>150 节点或 >800 边)物理参数专门调过**:斥力从默认视图的 -190 弱化到 -55、连接距离从 54 收到 36、向心力从 0.08 提到 0.4——直接照搬默认视图参数会导致孤岛/边缘节点被推得很远、画布大片黑边空着(未调前的真实反馈:「不利于充分利用空间」);同时大图渲染改冷启动定格(同步跑到仿真真正收敛——alpha 降到 alphaMin 以下,而非猜一个固定 tick 数;猜早了(曾用固定 90 tick)配合弱化过的斥力根本收敛不完,碰撞力(collide)没机会撑开,新长出来的节点挤成一坨、圆圈文字互相重叠,400 tick 是防跑飞的安全上限;不逐帧动画重渲染上千 DOM 节点,此前撞过真实卡死)。三态提示条改「说明胶囊 `.trunc-label`」+「独立药丸按钮群 `.trunc-action`」两部分,不再是一整块文字挤一个胶囊分不清哪段能点(冒烟反馈"太粗糙"),按钮跟侧栏「全局图谱」同一套 hairline 描边视觉语言,「显示全部」这种一步到底的重操作用 accent 底色跟「继续展开」「收起」区分开。**节点=名字本身**(不是色块):半径按重要度算(note_count 归一,MIN_R 12→MAX_R 38),标签**逐字型估宽**装进圆内(CJK/全角≈1em、ASCII≈0.55em,按实际渲染字号算可用宽度,装不下逐字截断加「…」,`title` 全名兜底)——统一按字符数估宽会让纯英文名[agent]被当中文一样宽而过度截断、又让长中文标题[笔记标题]按英文宽度算能装下却溢出糊到邻居,故必须分字型,标签白底反色文字居中渲染在圆内——圆圈本身即标签,不依赖 hover/最大若干个才显示。**节点色**:人实体按个体 `speakerInk`(与会议搭子同色系,跨页认人);非人按 `kindInk(kind)`(`$lib/graph` 导出,全应用 kind 上色唯一真值源,跟侧栏 kind 过滤药丸/实体列表色点/详情面板 kind 徽章同一份取色逻辑,不会各画各的对不上号)。**画布右下角图例**:按当前渲染集合里实际出现的类别生成(kindInk 色点+`kindLabel` 中文名);人物没有单一代表色(按个体区分),图例用四色渐变点+「人 · 按个人上色」说明,不能瞎编一个色骗大家。**镜头**:滚轮缩放(围绕光标定位,0.3x~8x)+ 拖空白处平移,叠在自适应 `fit` 变换之外的第二层(`viewZoom`/`viewX`/`viewY`),数据集变化(换中心实体/kind 过滤/展开)时归零回自适应总览。节点拖拽(指针按住跟随、松开回归仿真,落点要经镜头+自适应两层变换逆运算)、hover 高亮邻居其余淡化(opacity)、点击导航(位移小于阈值判点击而非拖拽;人→会议搭子,非人→选中态)、右键节点弹改名菜单(见下)。`prefers-reduced-motion` 时跳过动画、仿真直接冷却定格。**kind 过滤联动画布**(`graphFilter.svelte.ts` 跨组件共享 `$state`,侧栏药丸与主区画布同源):点药丸不再只筛侧栏列表,画布本身也只保留该 kind 的节点(边要求两端都保留),筛完不足 2 个节点显示「没有匹配的实体 · 清除筛选」占位而非空白画布。**搜索定位**:侧栏搜索框同一份 `graphFilter.query` 经 `query` prop 传入,命中节点(名字/别名子串)画布上描边高亮+其余淡化+镜头自动聚焦(bbox 居中+适度放大,封顶 3.5x);只在当前已渲染的节点里找,被封顶挤掉的实体要先点「显示全部」。
