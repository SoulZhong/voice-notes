...
- **图谱页**（侧栏第三页签「图谱」，消费 Aing 语义知识图谱）：它是笔记关系的轻量地图，不是图数据库工作台。日常界面只保留实体搜索、实体类型、点击节点逐步展开、实体详情与关系证据；不提供文章/实体双视角、时间与关系类型组合筛选、历史开关、两点路径、待整理队列或全量浏览。语义索引不可用或待补建时明确显示降级原因，并提供「补建语义关系」入口；失败不会阻断笔记录制、保存、精修或阅读。
- **force-graph**（图谱页主区，d3-force + SVG 自绘，`ForceGraph.svelte`）：顶点和边直接承载**完整可读标签**，不使用省略号或脱离顶点的常驻说明方框；长名称在顶点内多行排布，完整关系名沿边显示，缩放时逐级显示更多完整的顶点名称和关系标签。实线箭头表示模型提取或人工确认的明确关系，常驻高对比图例说明方向并提示可点击查看依据；虚线仅表示同篇笔记共同出现，明确声明它不等于已确认关系。别名继续用于搜索，证据与治理状态通过详情面板查看。顶点可拖拽，空白处平移，滚轮围绕光标缩放；hover 聚焦邻域，点击节点展开一层，键盘可达，`prefers-reduced-motion` 时直接定格。大图默认只显示关系主干，不以截断文本换取空间。
