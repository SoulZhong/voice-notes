...
- **force-graph**(图谱页主区,d3-force + SVG 自绘,`ForceGraph.svelte`):节点=实体、边=共现(权重=共享笔记数)。**规模封顶**:926+ 实体不可全画,只取「共享≥2篇」的强边算度数,按 note_count 降序取前 60 节点,每节点只保留最强 3 条边(backbone 稀疏化,把千级共现边收成可读骨架)、超出角标提示「显示连接最紧的 N 个 · 用左侧列表看全部」。**节点=名字本身**(不是色块):半径按文字宽度算(短名字最小半径 16px,装不下就放大到 30px 封顶,再装不下才截断加「…」,`title` 全名兜底),标签白底反色文字居中渲染在圆内——圆圈本身即标签,不依赖 hover/最大若干个才显示。节点色=人实体 `speakerColor` 会议搭子色系、非人 `ink-faint` 中性;拖拽(指针按住跟随、松开回归仿真)、hover 高亮邻居其余淡化(opacity)、点击导航(位移小于阈值判点击而非拖拽;人→会议搭子,非人→选中态)。`prefers-reduced-motion` 时跳过动画、仿真直接冷却定格。
