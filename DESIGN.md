...
- **智能精修配置**(AI 页首卡,.config 内嵌面板):首行「执行方式」segmented 二选一——在线接口 / 本机 Agent。在线接口态:一键填充 `button-secondary` 预设行 + 三字段(接口地址/模型/API Key);本机 Agent 态:「Agent」segmented 四选一(Claude Code/Codex/Gemini/Cursor,分段项只放名字不带状态记号),仅当前选中家的探测状态以 caption 级 `ink-faint` 尾随分段选择右侧(已找到 <路径> / 未找到=`warning-ink` / 填了路径覆盖时「使用指定路径」);两个可选字段(模型:占位符按所选家给示例;CLI 路径:占位符「留空自动探测」,不得把探测结果当占位符伪装成已填值);config-hint 一行大白话(可用=无需 Key、走本机登录与额度、未登录会失败保原文;不可用=先装并登录或填路径),路径只出现一次不与状态行重复。启用总开关仍在设置页「录制」区。
