...
- **force-graph**（图谱页主区，d3-force + SVG 自绘，`ForceGraph.svelte`）：顶点和边直接承载**完整可读标签**，不使用省略号或脱离顶点的常驻说明方框；长名称在顶点内多行排布，常用规模下关系名沿边直接显示，大图缩放时逐级显示更多完整的顶点名称和关系标签。明确关系使用高对比 accent 粗实线与箭头，弱连接使用可辨认的次级色虚线；常驻图例同时解释方向和虚线语义。别名继续用于搜索，证据与治理状态通过详情面板查看。顶点可拖拽，空白处平移，滚轮围绕光标缩放；hover 聚焦邻域，点击节点展开一层，键盘可达，`prefers-reduced-motion` 时直接定格。大图默认只显示关系主干，不以截断文本换取空间。
