...
- **force-graph**（图谱页主区，d3-force + SVG 自绘，`ForceGraph.svelte`）：顶点和边直接承载**完整可读标签**，不使用省略号或脱离顶点的常驻说明方框；长名称在顶点内多行排布，完整关系名沿边显示，缩放时逐级显示更多完整的顶点名称和关系标签。别名继续用于搜索，证据与治理状态通过详情面板查看。顶点可拖拽，空白处平移，滚轮围绕光标缩放；hover 聚焦邻域，点击导航，键盘可达，`prefers-reduced-motion` 时直接定格。路径计算使用确定性的关系成本、稳定并列顺序，并始终允许从路径边进入原始证据。大图通过骨架、逐层展开和显式「显示全部」控制密度，不以截断文本换取空间。
