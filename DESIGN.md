...
- **AI 调用日志**(AI 页末卡,section-title「AI 调用日志」,settings-row):每条一行——row-label=类别中文名(精修分块/标题生成/Agent 精修/精修写回),失败时尾随 `pill.warn`「失败」徽章、超长截断时尾随中性 pill;row-desc 一行元信息「MM-DD HH:mm · 执行方 · 模型 · 耗时 · 笔记 id」,失败追加 `warning-ink` 错误摘要(≤80 字);行尾 `button-link`「详情/收起」,展开为行下 `.snippet` 同族 JSON 全文(限高 20rem 内滚,pre-wrap)。卡底操作行:row-desc 给「共 N 条,此处显示最近 20 条 + CLI 指引」或导出结果路径,右侧 `button-secondary`「刷新」「导出 JSONL」(空库时导出禁用)。空态一行 config-hint 说明留痕机制。
