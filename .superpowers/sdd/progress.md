...
## 段内说话人分离 B(分支 intra-segment-diar)执行
- Task 1: complete (eb0beab..e892a2a, review Approved) — Transcript 透传 tokens/timestamps
- Task 2: complete (e892a2a..5b9d3b1+标注补齐, review Approved, 数学手工验算全过, 10 新测) — split.rs 纯逻辑
- Task 3: complete (5b9d3b1..ccf528e, review Approved; 回退路径逐条走查) — worker 接线
  - Minors(→终审 triage): 切分时末子段可能丢 <1ms 音频尾(ms 换算下取整,可用 len 修正); 实现者报告既有测试数 27 实为 20(cosmetic)
- 全分支终审 (fable): With fixes → 修复(27a8244: ▁BPE 清理/回退子段过语言过滤/ECHO contains 门槛 4 字符/耗时日志/末子段全尾/total_cmp/spec 修订入库)→ 复审 Ready to merge YES
- B backlog(复审残留+终审记录): 回退子段语言过滤可换用重识别自身 t.lang(严格更强一字改); 无变更点长段的滑窗成本无日志(校准 partial 饥饿时补); ECHO 两路不对称切分漏杀面(带回声素材观察); 滑窗 hop 750ms 降耗/长段间隙插 partial 服务; 子段短样本拖质心(并入阈值校准)
- 全量验证: cargo test 159 passed(B 新增 17), npm check 0/0
- 冒烟修复(70adb56+可见性收窄): AEC 残渣抑制(rms<0.012 且与 system 重叠 ≥0.8,首批数据校准)+ 15s 硬切找静点
  - 已知风险(记账): rms 灰区(0.012~0.02)的远场轻声真人插话若与外放重叠 ≥80% 会被误判残渣丢弃——静默丢真实内容,更多场次数据后复盘阈值; 补充测试项: 切分后 mic 子段逐个过残渣判定(代码走查为真,缺测试)

## ✅ B 段内说话人分离 squash 合入 master (2026-07-05, c5a19e8, PR #11)
含冒烟修复: AEC 残渣抑制(rms<0.012+重叠≥0.8)与 15s 硬切找静点。挂起校准项: 残渣 rms 灰区、短段建簇(<2.5s 裂人)、ECHO 不对称漏杀观察、滑窗 hop 750ms。

## ✅ 音频保留+播放+声纹试听 squash 合入 master (2026-07-05, fd36c23, PR #12)
每源 16k s16 WAV 旁路落盘(与段时间轴按构造对齐;写盘独立线程;audio.json offset 支持轨道中途出现;头 1s 回写+启动扫描修陈旧头);详情页多轨播放器(高亮跟随/点时间戳跳播/图标按钮右上);声纹库样本 voiceprints/<P#>.wav(≥10s 定格截 15s,只写首个,VP_LOCK 串行,merge 继承/delete 连删)+ 管理页重做(头像行/图标操作/试听动效/合并弹出菜单/已命名·待命名分区)。8 角度审查修复:audio.json 并发建档竞态、修头 TOCTOU(改启动扫描)、无锁样本写入、__invalid__ 共享兜底、offset 丢失固化、u32 头回绕、前端依赖未追踪/响应过期。已知取舍(spec): CSP 仍 null、WAV 手写头、样本 map 在 ASR worker。验证: cargo test 182 过, svelte-check 0/0。磁盘 ≈115MB/h/源,压缩留后续。
