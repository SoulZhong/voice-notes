...
- Task 3: complete — worker 接线:recognize 后取 (text, lang) 过滤外语幻觉段(与 ECHO 命中同待遇,不 embed/不 assign/不 emit/不落盘);段级 rms 经 PendingMic/process_final/on_final 贯通至 lib.rs 落盘;is_foreign_final 收紧为私有、去 allow(dead_code)(T1 遗留 Minor 清理)
...
- Task 3: complete (51fc6ba..03d08d1, review Approved, 零 issue; T1 pub 遗留已收紧) — worker 过滤+rms 接线
