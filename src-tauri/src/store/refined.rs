...
pub struct RefinedParagraph {
    ...
    /// 手动关联。有它才能把 Aing 稿改名同步进声纹库(会议搭子)。
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// 丢更新(与 notes.rs EDIT_LOCK 同一哲学,独立锁——Aing 稿编辑与笔记编辑互不相干)。
...
/// (编辑必须以「盘上有可编辑的 Aing 稿」为前提,不能凭空造一份)。
fn update_refined(
    note_dir: &Path,
    ...
    let mut doc =
        load_refined(note_dir).ok_or_else(|| anyhow::anyhow!("Aing 稿不存在或已损坏"))?;
    f(&mut doc)?;
    write_refined_atomic(note_dir, &doc)
}

/// Aing 稿说话人改名:该 speaker 的全部段落 name 置为新名。
...
pub fn rename_refined_speaker(
    ...
    update_refined(note_dir, |doc| {
        ...
        anyhow::ensure!(hit, "Aing 稿中没有该说话人: {speaker_id}");
        Ok(())
    })?;
    Ok(person_id)
}

/// 把 Aing 稿说话人关联到声纹库人物:该 speaker 的全部段落写入 person_id,
...
/// 只读 join:关联了库人物的段落,展示名跟随库中现名(会议搭子改名 → 历史 Aing 稿
