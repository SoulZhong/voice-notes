...
此外,抽取本批出现的关键实体(不改动正文),用修订后的规范名。**不要输出关系(relations)**:关系由独立的整篇抽取阶段负责,在这里产出只会白占输出长度并把 JSON 顶到 token 上限。
...
texts 数组长度必须与输入段落数一致,顺序一致。glossary 只收实体类归一项;只收实体类的错写→正写,一条就是一个词,不要把多个词连成一条。entities 没有可给空数组,aliases 可省略。"#;
