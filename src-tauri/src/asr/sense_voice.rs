...
impl SenseVoiceRecognizer {
    ...
    pub fn new(model_dir: &Path, provider: Option<String>) -> anyhow::Result<Self> {
        ...
        let inner = OfflineEngine::new(&spec, default_threads(), provider.as_deref())
...
/// CPU 推理最便宜的提速是多线程:按可用并行度取值,上限 8。三个识别器共用。
...
/// 在目录中找到 tokens.txt。
pub(crate) fn find_tokens(dir: &Path) -> anyhow::Result<std::path::PathBuf> {
