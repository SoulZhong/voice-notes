            ...
            std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                ...
                if let Some(identify_exec) = identify_exec {
                    ...
                    let identify_result = (|| -> anyhow::Result<()> {
                        ...
                        identify_actions::recover_identify_ops(&TauriEnv(app.clone()), &note_id);
                        ...
                        if s.identify_auto_apply {
                            let fps: Vec<String> = store::load_refined(&dir)
                                ...
                                .unwrap_or_default();
                            for fp in fps {
                                if let Err(e) = identify_actions::auto_apply_one(&TauriEnv(app.clone()), &note_id, &fp) {
                                    eprintln!(
...
use split_flow::{
    auto_split_speaker_with, cancel_split_with, commit_split_with, confirm_multi_samples_with,
    mark_speaker_multi_with, resolve_multi_residual_with, suggest_split_groups_with,
...
impl voice_env::VoiceEnv for TauriEnv {
    ...
    fn edit_note(&self, op: lifecycle::machine::EditOp) -> Result<(), String> {
        self.0
            .state::<lifecycle::LifecycleHandle>()
            .request(lifecycle::machine::Msg::EditNote { op })
    }
    fn admit(&self, note_id: &str, intent: occupancy::Intent) -> Result<(), String> {
        admit_note(&self.0, note_id, intent)
    }
...
impl SplitEnv for TauriEnv {
    ...
    fn begin_exclusive_rebuild(&self) -> bool {
        ...
        !REBUILD_RUNNING.swap(true, std::sync::atomic::Ordering::SeqCst)
    }
    ...
    fn consume_pending_rebuild(&self) {
        ...
        let go = {
            let _ctl = REBUILD_CTL.lock().unwrap();
            REBUILD_PENDING.swap(false, std::sync::atomic::Ordering::SeqCst)
        };
        if go {
            let st = self.0.state::<AppState>();
    ...
    fn split_progress(&self, note_id: &str, done: usize, total: usize) {
        let _ = self.0.emit(
            "auto_split_progress",
            serde_json::json!({ "note_id": note_id, "done": done, "total": total }),
        );
    }
}
...
async fn identify_note(app: AppHandle, id: String) -> Result<(), String> {
    ...
    tauri::async_runtime::spawn_blocking(move || -> Result<(), String> {
        let run = || -> anyhow::Result<()> {
            ...
            let now = chrono::Local::now().to_rfc3339();
            identify_actions::recover_identify_ops(&TauriEnv(app2.clone()), &id); // 无条件前置(理由同管线路径)
            let idoc = {
                let _gate = IDENTIFY_ACT_GATE.lock().unwrap();
                let idoc = refine::identify::run_identify(
                    &dir,
            ...
            if s.identify_auto_apply {
                let fps: Vec<String> = store::load_refined(&dir)
                    ...
                    .unwrap_or_default();
                for fp in fps {
                    if let Err(e) = identify_actions::auto_apply_one(&TauriEnv(app2.clone()), &id, &fp) {
...
/// 收件箱身份建议列表 + 自动认人回执(规则见 identify_actions)。
#[tauri::command]
fn list_identify_suggestions(app: AppHandle) -> Result<Vec<ipc::IdentifySuggestion>, String> {
    identify_actions::list_identify_suggestions_with(&TauriEnv(app))
}

/// 回执「好」:确认自动认人,补做确认才入库的回灌与样本(identify_actions)。
#[tauri::command]
fn acknowledge_identify(app: AppHandle, note_id: String, op_id: String) -> Result<(), String> {
    identify_actions::acknowledge_identify_with(&TauriEnv(app), note_id, op_id)
}
...
/// 确认身份建议(identify_actions::apply_identify_suggestion_with)。
#[tauri::command]
async fn apply_identify_suggestion(app: AppHandle, note_id: String, fingerprint: String) -> Result<(), String> {
    let env = TauriEnv(app);
    tauri::async_runtime::spawn_blocking(move || {
        identify_actions::apply_identify_suggestion_with(&env, note_id, fingerprint)
    })
    .await
    .map_err(|e| e.to_string())?
}

/// 拒绝身份建议(identify_actions::reject_identify_suggestion_with)。
#[tauri::command]
fn reject_identify_suggestion(app: AppHandle, note_id: String, fingerprint: String) -> Result<(), String> {
    identify_actions::reject_identify_suggestion_with(&TauriEnv(app), note_id, fingerprint)
}
...
fn set_segment_speaker(
    ...
    if is_active_note(&state, &note_id) {
        ...
        // "none"/"multi"(清除归属/标记多人)同拒:live 路径的 writer 只认已注册
        // S-id,哨兵会被当未知说话人报错,这里先给出面向用户的原因。
        if speaker_id == "none" || speaker_id == "multi" {
            return Err(tr!(
                "录制中不能修改段落归属，请先停止录制",
                "Cannot change a segment's assignment while recording"
            ));
        }
