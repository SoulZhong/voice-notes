...
/// 关联了库人物的段落做只读 join：展示名跟随声纹库现名（会议搭子里改名 → 历史 Aing 稿
...
/// Aing 稿说话人改名，并同步声纹库（会议搭子）：该说话人已关联库人物时，库中人名一并
/// 更新——所有历史与未来会议随之显示新名；未关联的只改本篇 Aing 稿。Aing 中拒绝（管线
...
fn rename_refined_speaker(
    ...
    // 降级 Aing 稿沿用 S* 标签(重聚类 skipped/failed):speakers.json 同名条目一并改,
    // 原始逐字稿视图不与 Aing 稿打架。R* 标签不存在于 speakers.json,不碰。
    if speaker_id.starts_with('S') {
        if let Err(e) = store::NoteStore::new(root).rename_speaker(&note_id, &speaker_id, name) {
            eprintln!("Aing 稿改名已生效,但同步 speakers.json 失败({speaker_id}): {e}");
        }
    }
    ...
    if let Some(pid) = person_id {
        ...
        if let Some(resolved) = store::VoiceprintStore::resolve(&vp, &pid).map(str::to_string) {
            if let Err(e) = vp_store.rename(&resolved, name) {
                eprintln!("Aing 稿改名已生效,但同步声纹库失败({pid}): {e}");
            }
        }
    }
    Ok(())
...
/// 把 Aing 稿说话人关联到声纹库人物（会议搭子选人）：段落写入 person_id 并采用库中
...
/// 导出笔记。prefer_refined=真且 Aing 稿在盘时导 Aing 稿(所见即所得:用户看着哪个视图
/// 点导出就得到哪个),否则导原始逐字稿;Aing 稿导出前与 get_refined 同款只读 join,
/// 库中现名(会议搭子改名)一并带出。
#[tauri::command]
