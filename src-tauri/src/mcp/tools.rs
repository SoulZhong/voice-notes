...
/// refined.json 存在时返回 Aing 稿(结构化给 paragraphs,md/txt 现场渲染 Aing 段)。
pub fn get_note(
    roots: &DataRoots,
    id: &str,
    format: &str,
...
// Aing 稿的 md/txt 渲染已下沉 store::export::render_refined(GUI 导出与此处共用,防漂移)。
...
pub fn apply_refined_texts(
    ...
    let result = (|| -> anyhow::Result<serde_json::Value> {
        ...
        anyhow::ensure!(
            ...
            "该笔记还没有 Aing 稿:请先在 App 里完成一次 Aing(或等停止录制后自动 Aing),再写回修订"
        );
...
mod tests {
    ...
    fn list_notes_pages_and_filters_by_time() {
        ...
        // 三月会补一份 Aing 稿:断言 has_refined 能区分有/无。
        ...
        assert_eq!(v["notes"][0]["has_refined"], true, "三月会已落 Aing 稿");
        assert_eq!(v["notes"][1]["title"], "一月会");
        assert_eq!(v["notes"][1]["speaker_count"], 1);
        assert_eq!(v["notes"][1]["has_refined"], false, "一月会无 Aing 稿");
    ...
    fn get_note_segments_markdown_and_refined_preference() {
        ...
        assert_eq!(v["refined"], false, "无 Aing 稿回落原始");
        ...
        // 落一份 Aing 稿:prefer_refined=true 时取 Aing
