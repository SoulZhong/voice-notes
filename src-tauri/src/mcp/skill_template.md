---
name: voice-notes
description: 查询本机 voice-notes 会议笔记(实时转写+说话人识别)。当用户询问会议内容、要会议纪要、写周报/日报需要汇总会议、找会上的决议/待办/承诺/时间点时使用。支持全文检索、读取全文(优先 AI Aing 稿)、录制状态查询与(需用户开启)录制控制。
---
...
- `get_note` 默认 prefer_refined=true:有 AI Aing 稿(错字修正/段落归并)时返回 Aing 稿,响应的 `refined` 字段标注来源;需要逐句时间戳或原始逐字稿时用 format="segments"、prefer_refined=false。
...
4. **代 Aing**(用户明确要求时):`get_note(format="segments")` 拿 Aing 稿 paragraphs → 只做错字纠正/实体统一/去语气词/中英排版,禁止改写语义 → `apply_refined_texts` 按下标提交有改动的段落(整段全文),确认无需修订则提交空 updates。笔记须已有 Aing 稿(refined=true),否则请用户先在 App 里 Aing 一次。
