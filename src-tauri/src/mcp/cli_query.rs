...
fn render_notes_human(v: &serde_json::Value) -> String {
    ...
    let mut out = String::from("id\t开始时间\t时长\t说话人\tAing\t标题\n");
    for n in &notes {
        out.push_str(&format!(
            "{}\t{}\t{} 分钟\t{}\t{}\t{}\n",
...
/// 标题素材:Aing 稿段落优先(错字已修、噪声已滤),没有 Aing 稿用原始 segments
...
mod tests {
    ...
    fn notes_cli_end_to_end_over_fixture() {
        ...
        // --raw:忽略 Aing 稿取原始逐字稿,fixture 无 Aing 稿故仅验证不因新 flag 报错
