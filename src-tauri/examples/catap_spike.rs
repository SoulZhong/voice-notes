...
mod mac {
    ...
    pub fn capture(args: &[String]) -> Result<()> {
        ...
        // 覆盖率要**逐轨**算(Codex 十八轮 P2):聚合设备的某一路(比如热插拔时 mic 那路)
        // 单独消失时,另一路还在推进,用 max 口径的总帧数看不出来,会落下一条被截断的轨
        // 却报成功。两轨各自的样本数都得够。
        let (mic_cov, sys_cov) = (shared.mic.len() as f64 / src_hz, shared.sys.len() as f64 / src_hz);
        if run < secs * 0.9 || mic_cov < secs * 0.9 || sys_cov < secs * 0.9 {
            bail!(
                "采集覆盖不足:请求 {secs:.0}s,实际首→末回调 {run:.1}s、mic 采到 {mic_cov:.1}s、\
                 system 采到 {sys_cov:.1}s(回调 {} 次)。某一路设备中途停摆/消失都会这样——\
                 本场不落盘、不出结论,但这条信息本身就是热插拔实验的观测数据。",
                shared.callbacks
            );
        }
