...
路线 A 在本机(macOS 26.5.2)**技术上完全跑得通,且成色好**:私有聚合设备 + 全局 CATap,
单 IOProc 同时出 mic 与系统输出两轨——两轨样本数逐帧相等、6 分钟零时间戳断层、
CPU 0.05% 单核;跨时钟域(内置麦 master + 蓝牙输出)的 **drift 补偿残余 ≲1ppm**
(实测 -3.26ppm 与 master 自身时钟误差 -3.1~-3.7ppm 在 0.6ppm 内吻合,而后者是两轨共模、
不产生轨间错位)。

**但裁决还不能下**,缺三块:判据 1 的声学口径(要"内置麦 + 外放"的场,本机当前输出在
蓝牙耳机上做不了)、判据 3 热插拔完全未测、判据 4 只测了终端身份没测签名 app。
另外走 A 就要为 13.x–14.1 维护双栈采集路径,这笔成本要与 B 的实现成本一起算。
...
| 项 | 第一场 | 第二场 |
| --- | --- | --- |
| 回调 / 帧 | 34700 / 17,766,400 | 34684 / 17,758,208 |
| **两轨样本数差** | **0** | **0** |
| **时间戳断层** | **0 次** | **0 次** |
| 回调耗时 p50/p95/max | 3 / 24 / 120 µs | 3 / 24 / 225 µs |
| CPU | 0.19s/370s ≈ **单核 0.05%** | 0.20s/370s ≈ **单核 0.05%** |
| 聚合设备时钟 vs `Instant` | (口径有 bug,见下) | **-3.7ppm** |
| 轨道电平 | mic rms 0.019 / system rms 0.064 | mic rms 0.013 / system rms 0.062 |
...
"两轨样本数差 0"不是巧合,是路线 A 的**结构性保证**:单 IOProc 一次回调同时给出两轨,
帧数天然相同。这正是它相对现状(mic 走 cpal、system 走 SCK,两条独立时钟)的根本优势。
...
2. **参考对照(本次做的)**:把播放用的参考文件与 system 轨互相关。它 =(补偿残余)+
   (master 时钟相对真实时间的误差)。后者本机实测约 3ppm(见 E2),所以这条给出的是
   补偿残余的**上界**,不是判据 1 本身。
...
四者落在 0.6ppm 内。也就是说这 -3.26ppm **整个由 master(内置麦)自身的时钟误差解释**,
而 master 误差对两轨是**共模**、不产生轨间错位。两块独立晶振(内置 vs 蓝牙)之间通常差
几十 ppm,这里完全没看到这一项——**drift 补偿把跨时钟域的差消到了 ~1ppm 量级**。

错位曲线的线性度同样是判据 2 的证据:6 分钟内最大偏离直线 0.042ms,没有任何台阶——
系统若做了内部硬校正,这里会看到跳变。

一个测量坑记在这:低相关窗(corr 0.1~0.2)的**峰位置仍然正确**,错位读数落在同一条斜坡上,
被压低的只是峰值。原因是探针那个无抗混叠的 48k→16k 抽取:相位随漂移缓慢滑移,
经过整采样点时重合度高、离开时下降。所以拟合用了全部 37 窗而不是只用高相关窗
(两者结果差 0.1ppm)。要根治就加 `--native` 按原始率落盘。

## 判据逐条对照(预注册,`clock-drift-sensor-design.md` 第五节)

| # | 判据 | 状态 |
| --- | --- | --- |
| 1 | 残余轨间漂移中位 <20ppm 且 1h 错位 <10ms | **强烈支持,未正式收口**:补偿残余 ≲1ppm(-3.26ppm 全由 master 共模时钟误差解释);声学口径待补 |
| 2 | 错位曲线无跳变(无系统内部硬校正) | **通过**:370s×2 场时间戳断层 0 次;错位曲线最大偏离直线 0.042ms |
| 3 | 设备热插拔不崩、行为可解释 | **未测** |
| 4 | TCC 授权体验可接受 | **部分**:从终端跑无提示、无拒绝,tap 直接建成并拿到真实系统音频;签名 .app 内的表现未测 |

判据 1 为什么说"未正式收口":本次量的是参考对照口径,它给出的是**补偿残余的上界**。
判据原文说的轨间错位要用声学口径(mic 真听见播放内容)直接量。两者的差别在于共模项——
参考对照里那 -3.26ppm 的共模误差在声学口径下会自动抵消,所以声学口径的读数只会**更好**,
不会更差。但判据是预注册的,不能拿"应该更好"顶替实测。
