...
**6 分钟零时间戳断层**(严阈值下复测,时间戳覆盖 100%);开销:IOProc 回调占用 0.05~0.07% 单核、本进程采集期 CPU
0.17~0.19% 单核,**coreaudiod 侧的 tap/聚合/重采样开销未测**(那才是路线 A 的主要成本项)。

**drift 补偿确实在工作**:同时钟域(内置麦 + 内置扬声器)的对照场读出精确的
+0.00ppm、且采集轨与参考文件逐样本一致;跨时钟域(内置麦 + 蓝牙输出)读出 -3.26ppm,
量级与"蓝牙域 − 内置域"的独立旁证相符(差 0.15ppm)。**但这不是残余的定量上界**
——那 0.15ppm 的吻合依赖"蓝牙栈按主机重定时"这个未在本机直接测量的前提。

**裁决不能下**,缺四块:判据 1 的**声学口径**(要 mic 真听见播放内容,且播放侧得在
**另一个时钟域**——同域场测了不说明补偿好坏)、播放侧时钟误差的独立测量、判据 3 热插拔
完全未测、判据 4 只测了终端身份没测签名 app。另外走 A 就要为 13.x–14.1 维护双栈采集路径,这笔成本要与 B
...
- 落盘按标称 48000 除三。所以**读数 ≈ (R_o − R_m)/48000**,即两个时钟域之差,
...
`corr 1.000` = 采集到的 system 轨与参考文件**逐样本一致**。这条对照排除了"整条量测链
(播放→tap→抽取→互相关)自己带漂移或根本不灵敏"的可能:同域时它读出精确的 0,
跨域时才读出非零。
