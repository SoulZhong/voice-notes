...
| 来源 | 值 |
| --- | --- |
| 本次参考对照斜率(31 个通过显著性检验的窗) | **-3.26 ppm** |
...
错位曲线的线性度同样是判据 2 的证据:6 分钟内最大偏离直线 0.037ms,没有任何台阶——
系统若做了内部硬校正,这里会看到跳变。
...
要根治相位滑移就加 `--native` 按原始率落盘(轨间比对不受影响,那条路两轨走同一条抽取)。

低相关窗能用、又不能什么都收,靠的是**峰显著性**而不是相关值本身:脚本用中位数+MAD
估背景涨落,只收高出背景 6 个稳健标准差的峰。这条门槛是必需的——Codex 实测过,
两条毫不相干的噪声轨在搜几千个滞后后也能凑出 0.02~0.03 的"峰",据此能拟合出
+2052ppm 的漂亮假结果;加上显著性检验后它们直接在粗对齐阶段被拒。
本次真数据 37 个窗里有 31 个过关,斜率与全收时一致(-3.26ppm)。

脚本另外挡掉的三类假通过(都拿实测/构造数据验过):全静音轨、恒定直流的"数字静音"
(不去均值的话每个滞后相关都是 1.0)、参考轨短于一个窗(过去会抛 NumPy 广播异常而不是
干净报错)。
