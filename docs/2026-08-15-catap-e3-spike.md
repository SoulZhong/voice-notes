...
四者落在 0.6ppm 内。**最自然的解释**是这 -3.26ppm 整个来自 master(内置麦)自身的时钟
误差,而 master 误差对两轨是**共模**、不产生轨间错位;按这个解释做减法得残余 ≈ +0.4ppm,
不确定度约 ±0.6ppm(几路共模估计彼此的离散度,且 `Instant` 的时基本身也不是标准钟)。

**这不是证明。** 严格说,观测量 = 补偿残余 + 共模项之和,理论上可以是"残余 +3ppm 与
共模 -6ppm 相抵"这种巧合。支持前一种解释的是:两块独立晶振(内置 vs 蓝牙)之间通常差
几十 ppm,若补偿没生效,这一项会以几十 ppm 的斜率直接暴露出来,而实测完全没有。
把话说死要靠声学口径——**那条口径里共模项按构造抵消,量到的就是残余本身**。
