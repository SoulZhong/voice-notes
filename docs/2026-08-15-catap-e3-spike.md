...
1. **声学口径的判据 1**:内置麦 + 一个**不同时钟域**的外放(USB/有线音箱,或摆在麦克风
   旁的蓝牙耳机)、安静房间、≥10 分钟,播 E1 标定序列,`xcorr_align system.wav mic.wav`
   取斜率。10 分钟能分辨到 ~2ppm(1 样本 = 62.5µs)。
   **注意别用内置扬声器**:它与内置麦同一时钟域,残余天然为 0(本次第三场对照已实测
   +0.00ppm),测了不说明补偿好坏。
