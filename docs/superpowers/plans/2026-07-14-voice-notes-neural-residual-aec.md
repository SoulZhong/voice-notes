...
- 模型工件哈希钉死(下载后校验,与 whisper 工件同哲学):
  - `dtln_aec_256_1.tflite` (5.5MB) sha256 `4a3a588b69fd79d837bc068b579a26faa92cac39dddbb00001d2dc1c3d869d60`
  - `dtln_aec_256_2.tflite` (10MB) sha256 `fa2590243aad1bf893c5be45b20709e8c50feec65e3604d1d52bae6eeddc23d3`
  - URL 前缀 `https://github.com/breizhn/DTLN-aec/raw/main/pretrained_models/`
...
### Task 1: 可行性硬闸——tract-tflite 加载并跑通一块

**Files:**
- Modify: `src-tauri/Cargo.toml`(`tract-tflite = "0.23"`;若该版本号不存在按 crates.io 最新 0.2x)
- Create: `src-tauri/src/audio/neural_aec.rs`(仅骨架:加载+单块前向)
- Modify: `src-tauri/src/audio/mod.rs`(挂 `pub mod neural_aec;`)

**Interfaces:**
- Produces: `pub struct DtlnAec { /* 两个 tract Plan + 状态形状 */ }`;`pub fn load(models_dir: &Path) -> anyhow::Result<DtlnAec>`(找 `dtln_aec_256_1.tflite`/`_2.tflite`,缺文件报错带路径)
...
Run: `cd src-tauri && VN_DTLN_DIR=/private/tmp/claude-501/-Users-teemo-workspace-soul-voice-notes/9d8a6b78-85d9-44bc-856b-738c3afa5d39/scratchpad/DTLN-aec/pretrained_models cargo test neural_aec -- --ignored --nocapture`

实现要点:
- `tract_tflite::tflite().model_for_path(..)`,打印双模型全部输入/输出 name/shape(eprintln,T2 依赖);
- 状态张量形状从模型声明读取,全零初始化;
- 单块:按「官方块算法」一节走一遍(FFT 用 realfft;此步一并加 `realfft = "3"` 依赖)。

**硬闸判定:** 若 tract 因算子不支持无法加载/运行(记录确切报错)——**切 tflitec 路线**:`tflitec = "0.7"` 替换依赖,`load` 内部改 tflitec Interpreter(API:`Interpreter::with_model_path`,`allocate_tensors`,`input/output tensor` 读写),对外接口不变;把切换原因写进模块头注释与任务报告,后续任务照常。**禁止**在两条路线都失败时继续——报 BLOCKED。
...
git commit -m "神经残余级可行性硬闸:tract-tflite加载DTLN-aec双模型单块出数(或按报告切tflitec)"
```
...
- [ ] **Step 1: 照抄 silero 条目形状增加(不压缩包,两个裸文件 = 两个 Artifact 或一个多文件 Artifact,以现有结构最贴近者为准)**
...
```rust
    ...
    if dtln_dir.join("dtln_aec_256_1.tflite").exists() {
