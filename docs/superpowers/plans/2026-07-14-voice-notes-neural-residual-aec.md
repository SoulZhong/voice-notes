...
- **落地偏差(审查跟进,commit f6707fb)**:`clean_wav` 与 `clean_mic_before_encode` 增显式 `models_dir: &Path` 参数(生产调用点传 `models::root()`,测试传自己的 tempdir)。原设计让神经级直接读全局 `models::root()`,而测试进程回落到本机 `src-tauri/models/` 开发目录——T5 一旦把 dtln 模型拷进去,合成单测会悄悄变成真推理测试且 `assert!(!report.neural)` 永久翻红(审查实锤)。参数化消灭全局状态依赖,零并发隐患。
