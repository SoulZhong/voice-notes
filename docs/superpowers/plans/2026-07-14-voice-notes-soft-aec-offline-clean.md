...
Run: 近期内置扬声器录音（具体笔记本地选取,id 不入库）同法跑
...
- **T6 标定结果**(commits 581c41e/b1ddab8):`PEAK_GATE=0.30`(三蓝牙场正例 vs 内置反例几何中点),`CONFIDENCE_GATE=2.0` 保持(内置反例 conf 经 second≤1e-6 分支爆表成千上万,分布无中点可取,分离全靠 peak;控制者签认)。标定工具输出路径 `VN_CALIBRATE_OUT` 环境变量化。**重要实测发现**:文档实锤的 2026-07-08 蓝牙场(正例C,具体笔记见本地标定报告,不入库)实为延迟漂移场——延迟 250~1200ms 散布,18 有效窗仅 3 过闸,并非稳定 600ms;稳定延迟场(正例A/B,全窗近匀,peak 中位 0.65+)才是离线清洗的主效场景。漂移场为一期已知局限:过闸窗定段、未过闸窗按归段延迟处理,清不掉的残余不伤近端,文本去重兜底;根治属二期实时滑窗重估的目标形态。
