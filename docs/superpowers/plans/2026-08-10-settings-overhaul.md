...
- 真机冒烟(用户做,写进 PR 描述):①三种旧文件升级(无键→B/true→AB/false→B)+备份文件生成;②AEC 常态录一段回声表现;③蓝牙耳机提示出现且文案无死指向;④无屏幕录制权限时拒录+引导卡+跳系统设置;⑤保留期 30d 设定后启动清理生效且录制中笔记豁免;⑥逃生舱手改 capture_path=vpio 生效;⑦设置页六分区/高级折叠/全部 Segmented 双主题;⑧AI 未配置完成徽标与跳转;⑨损坏 settings.json 启动不丢凭证。
