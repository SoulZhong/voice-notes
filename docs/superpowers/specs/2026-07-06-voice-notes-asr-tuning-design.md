...
样本:2026-07-06 09:51 决策机合并金融团队会议(54 分钟,notes/20260706-095122),豆包同场纪要在 samples/。两边时间轴经文本对齐确认偏移≈0,可逐段对比。
...
**ASR 文本:底子不差,输在后处理**:
- 实体不一致最痛:决策机→角色机/角色耳机/决策局/决对级;资管→自管/投馆;泛子网→范子管/范丝官。豆包全程一致(热词+LLM 后处理)。
