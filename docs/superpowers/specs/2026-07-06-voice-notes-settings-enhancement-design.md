...
- **双源模式录外放的电平代价(冒烟发现,根因已查实)**:VPIO 启动即触发 macOS 通话模式 ducking,压低全系统外放约 12-16dB(QuickTime 同源对照实测),连带录音电平与声纹嵌入质量(说话人裂增);Min 档+动态配置已是最轻,属固有行为。**外放场景的正解是「仅系统声音」开关**(VPIO 不启动,ducking 不发生),设置页文案已点明;回放增益归一化记 backlog 兜底双源轻声录音。
