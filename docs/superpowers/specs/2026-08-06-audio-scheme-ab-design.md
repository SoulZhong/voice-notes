...
| 混音器任一环节异常(**续录场景**) | 停写,但**保留**一条尾部截断的 `mixed.wav`——删它会连带冲掉上一场已完好落盘的内容。该笔记因此**仍会被判定为有方案 B 产物**,`mixed_track()` 照样返回 `Some`,`duration_ms` 是"写到一半的长度" |
...
**截断轨没有盘上标记**:上面第二行那种"合法但截断"的 `mixed.wav`,还有另外两条产生
路径(非续录场景 `remove_file` 失败;混音线程 panic 时 `AudioTrackWriter::Drop` 照样
补完合法头而删除逻辑够不着)。三条路径的唯一线索都只是一行 eprintln,进程重启后连这行
都没有。一期无人消费这条轨,故刻意不加标记;**二期消费前必须自行校验,或先补上盘上
标记**(`set_track_sync` 是可照抄的模板)。详见 `mixed_track()` 的文档注释。
