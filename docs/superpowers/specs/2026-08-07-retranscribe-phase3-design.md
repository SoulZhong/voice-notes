...
- **离线回声去重**(新造,纯函数):实时链路的回声去重靠墙钟 hold 计时
  ...
  ≥ 阈值」,阈值初值在实施计划中定标并由单测锁死。仅双轨模式需要。
- **电平交叉门**(冒烟后补,2026-08-08):沙箱实测推翻了「清洗后的 mic 轨大多已无
  回声」的假设——受污染场次清洗后仍有**响亮的乱码残影**(样本场次 53/73 个 mic 段,
  rms 中位 0.056 远超实时残渣门限 0.012,乱码文本与 system 相似度中位 0.15,任何
  文本阈值都抓不住)。故复用 `player_gate` 的电平判据(回放跨轨门控,已实证误压
  <1%):双轨 PCM 并存时算 mic 压低区间(`build_gate_from_pcm` 新入口),mic 段落
  在区间内时长占比 ≥ `GATE_COVER_MIN`(0.6 初值)即弃,计入 echo_dropped。文本门
  抓「转写得清楚的回声」,电平门抓「乱码残影」,互补;单轨/Mixed 无从交叉,门降级
  不开(与回放门控降级口径一致)。
...
- **MCP 工具**两个,经 UDS 桥打到运行中的 App(照 `reaing` op 的既有模式):
  ...
  (note_id + 阶段,空闲返回 null);返回值含最近一次任务的终态 last(ok 带摘要/
  error 带原因),批量驱动靠它区分完成与放弃。摘要{旧段数、新段数、声纹命中数、继承数、
