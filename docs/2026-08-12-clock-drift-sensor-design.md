...
> 结论摘要:路线 A 在 macOS 26.5.2 上跑得通,单 IOProc 双轨每次回调帧数相等(43143 次
> 回调零例外)、IOProc 回调占用 ~0.06% 单核(coreaudiod 侧开销未测);同时钟域对照场
> 读出精确 0.00ppm,跨时钟域读出 -3.26ppm,与"补偿生效"的预期一致,但分离不出残余
> (两域时钟各自的误差都没独立量)。
> **四条判据没有一条完全收口**:判据 1 的声学口径未做;判据 2 只算部分——设备级时间戳
> 看不见 tap 内部的补偿动作,而严阈值的长跑只在同域场做过;判据 3 热插拔未测;
> 判据 4 只测了终端身份、没测签名 app。故**尚未裁决**。
...
> **二期裁决(2026-08-28,用户拍板:不立项,观测续行)**:E2 干净样本 24 场,
> |轨间漂移| P50 3.10ppm / P95 3.52ppm,设备间稳定——未修正的双轨错位 ≈ 12ms/h,低于
> 回放高亮的可感知阈值,也在 align.json 离线时基投影(#103)兜住的范围内。路线 A/B
> 的实现成本均为这点收益服务,E3 剩余三条判据(声学口径/热插拔/签名 app TCC)各需半天
> 真机实验,收益配不上。**保留一期传感器持续观测**(drift_report/drift_stats);
> **重启条件写死**:任一设备组合 P95 > 20ppm,或出现用户可感知的双轨错位投诉。
> 覆盖缺口备忘:AirPods/有线/USB 声卡组合仍为 0 场,若日后接入应先补 E2 再谈路线。
