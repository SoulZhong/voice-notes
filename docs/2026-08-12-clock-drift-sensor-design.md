...
> 结论摘要:路线 A 在 macOS 26.5.2 上跑得通,单 IOProc 双轨每次回调帧数相等、
> 370s×2 场零时间戳断层、IOProc 回调占用 ~0.06% 单核(coreaudiod 侧开销未测);
> 跨时钟域参考对照斜率 -3.26ppm,与"补偿生效"的预期一致,但分离不出残余(播放侧时钟
> 未独立量)。判据 1 的**声学**口径未做、判据 3 热插拔未测、判据 4 只测了终端身份,
> 故**尚未裁决**。
